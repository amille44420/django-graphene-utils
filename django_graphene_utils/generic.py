...
class BaseModelFormMutation(object):
    ...
    def get_form_kwargs(self, root, args, context, info):
        return {
            ...
            'instance': self.get_instance(root, args, context, info)
        }
...
class Options(object):
    def __init__(self, options=None):
        ...
        assert issubclass(self.form_class, forms.ModelForm), \
            '%r is an invalid form class' % self.form_class
...
"""
Meta class model form mutation
"""


class ModelFormMutationMeta(type):
    def __new__(mcs, name, bases, attrs):
        ...
        # get the meta class
        opts = new_class._meta = Options(getattr(new_class, 'Meta', None))
        ...
        new_class._output_attrs = output_attrs = {
            # the common fields
